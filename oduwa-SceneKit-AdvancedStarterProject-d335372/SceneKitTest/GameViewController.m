...
- (void) setupCharacter
{
    ...
    character = [[GameCharacter alloc] initFromScene:[SCNScene sceneNamed:@"art.scnassets/SHAFA1.DAE"] withName:@"SpongeBob"];
    ...
    // Get Walk Animations
    NSURL *url = [[NSBundle mainBundle] URLForResource:@"art.scnassets/SHAFA1.DAE" withExtension:@"dae"];
...
- (void) setupCamera
{
    ...
    cameraNode.position = SCNVector3Make(0,50, 150);//cameraNode.position = SCNVector3Make(0, 100, 0);
...
- (void)renderer:(id<SCNSceneRenderer>)aRenderer updateAtTime:(NSTimeInterval)time
{
    if(movementJoystick.velocity.x != 0 || movementJoystick.velocity.y != 0){
        ...
        character.characterNode.rotation = SCNVector4Make(0,0 , 1, movementJoystick.angularVelocity);
        ...
        NSLog(@"[%f: %f, %f, %f,%f]", angleInDegrees, forwardDirectionVector.x, forwardDirectionVector.y, forwardDirectionVector.z,movementJoystick.angularVelocity);
      ...
      //  character.characterNode.rotation = SCNVector4Make(1,0, -100, M_PI+movementJoystick.angularVelocity);
    }
    else{
        character.actionState = ActionStateIdle;
    }
